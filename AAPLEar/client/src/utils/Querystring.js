export default class QueryString {
  ...
  static stringify(obj, sep, eq, name) {
        ...
        .map(function (k) {
          ...
          if (Array.isArray(obj[k])) {
            return obj[k]
              .map(function (v) {
                return (
                  ks + encodeURIComponent(QueryString.stringifyPrimitive(v))
                );
              })
              .join(sep);
          } else {
            return (
              ks + encodeURIComponent(QueryString.stringifyPrimitive(obj[k]))
            );
          }
        })
        .join(sep);
    ...
    if (!name) return "";
    return (
      encodeURIComponent(QueryString.stringifyPrimitive(name)) +
      eq +
