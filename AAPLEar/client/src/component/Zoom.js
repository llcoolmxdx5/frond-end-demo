...
class Zoom {
  ...
  mouseHandler(e) {
    this.maxZoom.style.display = this.mask.style.display =
      e.type === "mouseenter" ? "block" : "none";
  ...
  maskMoveHandler(e) {
    ...
    if (this.maskLeft >= this.miniZoomRect.width - Zoom.MASK_WH)
      this.maskLeft = this.miniZoomRect.width - 2 - Zoom.MASK_WH;
    if (this.maskTop >= this.miniZoomRect.height - Zoom.MASK_WH)
      this.maskTop = this.miniZoomRect.height - 2 - Zoom.MASK_WH;
  ...
  updateZoomImg(src) {
    this.miniZoom.style.backgroundImage =
      this.maxZoom.style.backgroundImage = `url(${src})`;
  }
}
...
class Carousel {
  ...
  carouselBnHandler(e) {
    if (e.currentTarget.src.indexOf("left") > -1) {
      ...
      this.imgCon.style.left =
        -(this.imgCon.offsetWidth - this.imgCon.parentElement.offsetWidth) +
        "px";
    }
  }
...
export default class GoodsZoom {
  ...
  init() {
    let img = document.querySelector(
      "#app > div > div:nth-child(2) > div > ul > li > img"
    );
    let evt = new Event(Carousel.ZOOM_IMG_CHANGE_EVENT);
    evt.src = img.src;
    document.dispatchEvent(evt);
