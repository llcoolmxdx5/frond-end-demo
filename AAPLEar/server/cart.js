...
module.exports = class Cart {
  ...
  addToShoppingList() {
    ...
    data["image"] =
      "/src/assets/img/img-" +
      item["productImages"]["items"][0]["src"].slice(6);
    data["count"] = 1;
    data["selected"] = true;
  ...
  selectGoods() {
    ...
    for (let i = 0; i < this.shoppingList.length; i++) {
      if (this.shoppingList[i]["id"] === this.req["id"]) {
        if (this.req["select"] === "true")
          this.shoppingList[i]["selected"] = true;
        if (this.req["select"] === "false")
          this.shoppingList[i]["selected"] = false;
      }
    }
    return this.shoppingList;
  }
